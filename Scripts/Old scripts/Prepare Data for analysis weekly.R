...
# Week of year
mydata <- mydata %>% mutate(Week = week(EventDate))
...
dat_week <- mydata_wide %>% group_by(EstuaryName, AuthorisedFisherID, FiscalYear, CalYear, Week) %>%
  summarise(Response = sum(Response, na.rm=T), Total_Catch = sum(Total_Catch, na.rm = T),
            Effort_days = n_distinct(LogSheetEventID)) %>% mutate(Proportion_Flathead = Response/Total_Catch) %>%
  filter(Total_Catch>0) %>% filter(Proportion_Flathead <=1)

hist(dat_week$Proportion_Flathead)


write_csv(dat_week, "Data/Flathead net key estuary wide Weekly.csv")
...
mydata_wide <- read_csv("Data/Flathead net key estuary wide Weekly.csv")
...
for(i in 1:nrow(mydata_wide)){
  temp_wind <- wind_dat %>% filter(Estuary == mydata_wide$EstuaryName[i]) %>%
    filter(CalYear == mydata_wide$CalYear[i] & Week == mydata_wide$Week[i])
...
mod_dat <- mydata_wide %>% drop_na(Effort_days) %>% filter(Effort_days<8) %>% #filter(Response <300) %>%
...
write_csv(mod_dat, "Data/Flathead Env data to model Weekly.csv")
