...
# Load the data
data <- read_csv("Data/Flathead Env data to model Flathead Net Commercial Monthly.csv")
...
data2 <- data %>% filter(C_E < 200)# %>% filter(fisher != 60092) %>% filter(catch>0)
...
new_data <- expand.grid("monthF" = unique(data2$monthF),
                        ...
                        "estuary" = "Wallis Lake", #unique(data2$estuary),
                        #"lunar_phase" = median(data2$lunar_phase, na.rm = T),
                        "mean_wind" = median(data2$mean_wind, na.rm = T),
...
p1 <- ggplot(new_data2, aes((scaled_flow), fit)) + geom_line() + facet_wrap(~estuary, scales = "free_x") +
...
new_data4 <- expand.grid("monthF" = "6",
                         "scaled_flow" = median(data2$scaled_flow),
                         "prop_dusky" = median(data2$prop_dusky),
                         "days_since_start" = median(data2$days_since_start),
                         "estuary" = "Wallis Lake",
                         #"lunar_phase" = median(data2$lunar_phase),
                         "mean_wind" = seq(min(data2$mean_wind), max(data$mean_wind), 0.1),
                         "fisher" = data2$fisher[1],
                         "cal_year" = data2$cal_year[1],
...
ggsave("Flathead Net Monthly model outcomes.png", dpi=300, width = 21, height=28, units="cm")
