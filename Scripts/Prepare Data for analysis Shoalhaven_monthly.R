...
mydata <- read_tsv("Data/RAS Download Flathead Paper.txt")

table(mydata$EstuaryName, mydata$FishingMethod)
table(mydata$FiscalYear)

mydata <- mydata %>% filter(EstuaryName == "Shoalhaven River, Crookhaven River") %>%
  filter(FishingMethod == "Meshing net") %>% filter(FiscalYear != "2024/25")


# monthly proportion of flathead
monthly_sum <- mydata %>% group_by(CalYear, Month) %>% summarise(total_catch = sum(WholeWtFishOnlineKg, na.rm = T))

fh_month_tot <- mydata %>% filter(CommonName == "Dusky Flathead") %>%
  group_by(CalYear, Month) %>% summarise(total_flathead = sum(WholeWtFishOnlineKg, na.rm = T))


combo <- fh_month_tot %>% left_join(monthly_sum) %>%
  mutate(Prop_flathead = total_flathead/total_catch*100) %>%
  ungroup() %>% group_by(Month) %>% summarise(Prop_flat_mean = mean(Prop_flathead),
                                              Prop_flat_sd = sd(Prop_flathead),
                                              n=n(),
                                              Prop_flathead_se = Prop_flat_sd/(sqrt(n)))

ggplot(combo, aes(Month, Prop_flat_mean)) + geom_col() +
...
mydata <- mydata %>% filter(EffortQty >100) # remove unreasonable efforts (too small)
...
n <- mydata_wide %>% 
  group_by(AuthorisedFisherID, EventDate) %>% 
  summarise(n = n()) %>%
  filter(n > 1) # 43 records to fix

mydata_wide <- mydata_wide %>% 
  group_by(across(c(1,3:44,46:49))) %>%
  summarise(across(`Yellowfin Bream`:`Marbled Flathead`, \(x) sum(x, na.rm=T)))

n <- mydata_wide %>% 
  group_by(AuthorisedFisherID, EventDate) %>% 
  summarise(n = n()) %>%
  filter(n > 1) %>% mutate(FishID_Date = paste(AuthorisedFisherID,EventDate,sep = "_"))

mydata_wide <- mydata_wide %>% mutate(FishID_Date = paste(AuthorisedFisherID,EventDate,sep = "_")) %>%
  filter(!FishID_Date %in% n$FishID_Date) %>% select(-FishID_Date)
...
mydata_wide$Response <- mydata_wide$`Dusky Flathead` + mydata_wide$`Flathead (other)`
...
write_csv(mydata_wide, "Data/Shoalhaven wide.csv")

monthly_dat <- mydata_wide %>% group_by(EstuaryName, AuthorisedFisherID, Month, CalYear, FiscalYear) %>%
  summarise(Dusky_Flathead = sum(Response, na.rm=T),
            Days_fished = n(),
...
flow_dat <- read_csv("Data/Flow Data/Shoalhaven Rv Grassy Gully2 Gauge.csv") %>% dplyr::select(-Comments)  %>%
...
for(i in (1:nrow(mod_dat))){
  ...
  mod_dat$Mean_daily_Flow[i] <- mean(temp_dat$Mean_Discharge, na.rm=T)
  mod_dat$Max_daily_Flow[i] <- max(temp_dat$Mean_Discharge, na.rm=T)
  setTxtProgressBar(pb, i)
}
...
pb <- txtProgressBar(min = 0, max = nrow(mod_dat), style = 3)
for(i in (1:nrow(mod_dat))){
  temp_dat <- winds %>% filter(Estuary == mod_dat$Estuary[i]) %>%
    filter(Year== mod_dat$CalYear[i]) %>%
    filter(Month == mod_dat$Month[i])
...
write_csv(mod_dat, "Data/Flathead Env data to model Shoalhaven Commercial Monthly.csv")
